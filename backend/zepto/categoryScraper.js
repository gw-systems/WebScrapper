...
async function scrapeCategoryProducts(page, categoryUrl) {
    ...
    const maxScrollAttempts = 20; // Increased attempts
    ...
    while (scrollAttempts < maxScrollAttempts) {
        ...
        if (currentHeight === previousHeight) {
            // Double check by waiting a bit longer and trying one more time?
            // For now, assume end of list
    ...
    // Post-process products in Node.js context (ID generation)
    const processedProducts = products.map(p => {
        const crypto = require('crypto');
        const idString = `${p.name}_${p.brand}`.toLowerCase();
        const idHash = crypto.createHash('md5').update(idString).digest('hex');
    ...
    console.log(`Found ${processedProducts.length} products in category`);
