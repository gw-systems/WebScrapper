...
async function scrapeProducts(page, term) {
  ...
  try {
      ...
      responseHandler = async (response) => {
        ...
        if (type === "xhr" || type === "fetch") {
          try {
            ...
            // Avoid empty_search URLs
            if (!url.includes("empty_search")) {
              ...
              if (json &&
                ...
                console.log(`Captured Zepto product JSON from: ${url}`);
                resolve({ json, url });
              }
            }
          } catch (e) {
    ...
    // 3. Wait for data (Promise race between Network Setup and DOM)
    // Actually we just wait for the promise we set up
    ...
    if (interceptionResult.json) {
      ...
      console.log("JSON interception timed out, using HTML fallback");
      const loaded = await ensureContentLoaded(page);
      if (loaded) {
        return extractProductsFromHTML(page);
      }
    }
